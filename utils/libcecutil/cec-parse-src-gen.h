...
static const char *capability_discovery_and_control_usage =
	...
	"                                  Send CDC_HEC_INQUIRE_STATE message (" xstr(CEC_MSG_CDC_HEC_INQUIRE_STATE) ")\n"
	...
	"                                  Send CDC_HEC_REPORT_STATE message (" xstr(CEC_MSG_CDC_HEC_REPORT_STATE) ")\n"
	...
	"                                  Send CDC_HEC_SET_STATE message (" xstr(CEC_MSG_CDC_HEC_SET_STATE) ")\n"
	"  --cdc-hec-set-state-adjacent phys-addr1=<val>,hec-set-state=<val>\n"
	"                                  'hec-set-state' can have these values:\n"
	"                                      deactivate (" xstr(CEC_OP_HEC_SET_STATE_DEACTIVATE) ")\n"
	"                                      activate (" xstr(CEC_OP_HEC_SET_STATE_ACTIVATE) ")\n"
	"                                  Send CDC_HEC_SET_STATE_ADJACENT message (" xstr(CEC_MSG_CDC_HEC_SET_STATE_ADJACENT) ")\n"
	"  --cdc-hec-request-deactivation phys-addr1=<val>,phys-addr2=<val>,phys-addr3=<val>\n"
	"                                  Send CDC_HEC_REQUEST_DEACTIVATION message (" xstr(CEC_MSG_CDC_HEC_REQUEST_DEACTIVATION) ")\n"
	"  --cdc-hec-notify-alive          Send CDC_HEC_NOTIFY_ALIVE message (" xstr(CEC_MSG_CDC_HEC_NOTIFY_ALIVE) ")\n"
	"  --cdc-hec-discover              Send CDC_HEC_DISCOVER message (" xstr(CEC_MSG_CDC_HEC_DISCOVER) ")\n"
	...
	"                                  Send CDC_HPD_SET_STATE message (" xstr(CEC_MSG_CDC_HPD_SET_STATE) ")\n"
	...
	"                                  Send CDC_HPD_REPORT_STATE message (" xstr(CEC_MSG_CDC_HPD_REPORT_STATE) ")\n";
...
static const char *dynamic_audio_lipsync_usage =
	...
	"                                  Send REPORT_CURRENT_LATENCY message (" xstr(CEC_MSG_REPORT_CURRENT_LATENCY) ")\n"
	"  --request-current-latency phys-addr=<val>\n"
	"                                  Send REQUEST_CURRENT_LATENCY message (" xstr(CEC_MSG_REQUEST_CURRENT_LATENCY) ")\n";
...
static const char *one_touch_play_usage =
	...
	"                                  Send ACTIVE_SOURCE message (" xstr(CEC_MSG_ACTIVE_SOURCE) ")\n"
...
static const char *routing_control_usage =
	...
	"  --request-active-source         Send REQUEST_ACTIVE_SOURCE message (" xstr(CEC_MSG_REQUEST_ACTIVE_SOURCE) ")\n"
	"  --routing-information phys-addr=<val>\n"
	"                                  Send ROUTING_INFORMATION message (" xstr(CEC_MSG_ROUTING_INFORMATION) ")\n"
	"  --routing-change orig-phys-addr=<val>,new-phys-addr=<val>\n"
	"                                  Send ROUTING_CHANGE message (" xstr(CEC_MSG_ROUTING_CHANGE) ")\n"
	"  --set-stream-path phys-addr=<val>\n"
	"                                  Send SET_STREAM_PATH message (" xstr(CEC_MSG_SET_STREAM_PATH) ")\n"
	"  --active-source phys-addr=<val>\n"
	"                                  Send ACTIVE_SOURCE message (" xstr(CEC_MSG_ACTIVE_SOURCE) ")\n";
...
static const char *system_audio_control_usage =
	...
	"  --set-audio-volume-level audio-volume-level=<val>\n"
	"                                  Send SET_AUDIO_VOLUME_LEVEL message (" xstr(CEC_MSG_SET_AUDIO_VOLUME_LEVEL) ")\n"
	"  --user-control-pressed ui-cmd=<val>,has-opt-arg=<val>,play-mode=<val>,ui-function-media=<val>,ui-function-select-av-input=<val>,ui-function-select-audio-input=<val>,ui-bcast-type=<val>,ui-snd-pres-ctl=<val>,channel-number-fmt=<val>,major=<val>,minor=<val>\n"
	"                                  'ui-cmd' can have these values:\n"
	"                                      select (" xstr(CEC_OP_UI_CMD_SELECT) ")\n"
	"                                      up (" xstr(CEC_OP_UI_CMD_UP) ")\n"
...
static const char *system_information_usage =
	...
	"                                  Send REPORT_PHYSICAL_ADDR message (" xstr(CEC_MSG_REPORT_PHYSICAL_ADDR) ")\n"
	"  --give-physical-addr            Send GIVE_PHYSICAL_ADDR message (" xstr(CEC_MSG_GIVE_PHYSICAL_ADDR) ")\n"
	"  --set-menu-language language=<val>\n"
	"                                  Send SET_MENU_LANGUAGE message (" xstr(CEC_MSG_SET_MENU_LANGUAGE) ")\n"
	...
	"                                      has-set-audio-volume-level (" xstr(CEC_OP_FEAT_DEV_HAS_SET_AUDIO_VOLUME_LEVEL) ")\n"
	"                                  Send REPORT_FEATURES message (" xstr(CEC_MSG_REPORT_FEATURES) ")\n"
...
static const char *timer_programming_usage =
	...
	"                                      saturday (" xstr(CEC_OP_REC_SEQ_SATURDAY) ")\n"
...
static const char *vendor_specific_commands_usage =
	...
	"                                  Send DEVICE_VENDOR_ID message (" xstr(CEC_MSG_DEVICE_VENDOR_ID) ")\n"
...
void cec_parse_msg_args(struct cec_msg &msg, int reply, const cec_msg_args *opt, int ch)
{
	...
	case OptSetAudioVolumeLevel: {
		__u8 audio_volume_level = 0;

		while (*subs != '\0') {
			switch (cec_parse_subopt(&subs, opt->arg_names, &value)) {
			case 0:
				audio_volume_level = strtol(value, 0L, 0);
				break;
			default:
				exit(1);
			}
		}
		cec_msg_set_audio_volume_level(&msg, audio_volume_level);
		break;
	}
